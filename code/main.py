...
class myInfer(inferServer):
    ...
    def pre_process(self, data):
        ...
        bytesIO = BytesIO()
        img =  tifffile.imread(BytesIO(bytearray(img)))
        return img
    ...
    def predict(self, data):
        with torch.no_grad():
            img = torch.from_numpy(data).unsqueeze(0).cuda()
            img = (img / 255. - self.mean) / self.std
            # output2 = self.model(img)
            output3 = self.model(img)
...
if __name__ == "__main__":
    model = UnetIBN(encoder_pretrained=False)
